...
from game import COLORS, FACTIONS, PHASES, STRATEGY_CARDS, Game
    ...
    with ui.row().classes("w-full items-center justify-between"):
        ...
        with ui.row().classes("items-center gap-2"):
            ...
            for phase in PHASES:
                ...
                ui.button(phase, on_click=lambda p=phase: (game.set_phase(p), touch())).props(
                    "flat dense no-caps"
                ).classes("imperial text-[10px] px-2").style(
                    f"color:{BRASS if active else DIM};"
                    f"background:{BRASS + '22' if active else 'transparent'}"
                )
            ui.button("Next phase", on_click=lambda: (game.advance_phase(), touch())).props(
...
def turn_banner() -> None:
    ...
    secs = int(game.elapsed(p))
    with ui.element("div").classes("panel w-full p-3").style(
        f"border-color:{COLORS[p.color]}88; background:{COLORS[p.color]}14"
    ):
        with ui.row().classes("w-full items-center justify-between no-wrap"):
            with ui.row().classes("items-center gap-3"):
                ui.element("div").classes("rounded-full").style(
                    f"width:12px;height:12px;background:{COLORS[p.color]}"
                )
                with ui.column().classes("gap-0"):
                    ui.label(f"{p.name} is up").classes("imperial text-sm").style(f"color:{INK}")
                    ui.label(p.faction).classes("text-xs").style(f"color:{DIM}")
            ui.label(f"{secs // 60:02d}:{secs % 60:02d}").classes("num text-2xl").style(
                f"color:{COLORS[p.color]}"
            )
            ui.button("End turn", on_click=lambda: (game.next_turn(), touch())).props(
                "dense no-caps unelevated"
            ).classes("imperial text-[10px]").style(f"background:{BRASS};color:{VOID}")
...
def player_card(p) -> None:
    ...
    with ui.element("div").classes("panel p-3").style(
        ...
        with ui.row().classes("w-full items-center justify-between no-wrap mb-2"):
            ...
            with ui.row().classes("items-center gap-1 no-wrap"):
                ui.button(icon="remove", on_click=lambda p=p: (game.score(p, -1), touch())).props(
                    "flat dense round size=sm"
                ).style(f"color:{DIM}")
                ui.label(str(p.vp)).classes("num text-2xl w-8 text-center").style(f"color:{color}")
                ui.button(icon="add", on_click=lambda p=p: (game.score(p, 1), touch())).props(
                    "flat dense round size=sm"
                ).style(f"color:{color}")
        ...
        ui.select(
            options,
            value=p.strategy_card,
            on_change=lambda e, p=p: (game.assign_card(p, e.value), touch()),
        ).props("dense outlined dark options-dense").classes("w-full text-xs mb-2")
        ...
        with ui.row().classes("w-full justify-between no-wrap mb-2"):
            counter(
                ...
                p.trade_goods,
                lambda d, p=p: (
                    setattr(p, "trade_goods", max(0, p.trade_goods + d)),
                    touch(),
                ),
                BRASS,
            )
            counter(
                "Tactic",
                p.tactic,
                lambda d, p=p: (setattr(p, "tactic", max(0, p.tactic + d)), touch()),
                INK,
            )
            counter(
                "Fleet",
                p.fleet,
                lambda d, p=p: (setattr(p, "fleet", max(0, p.fleet + d)), touch()),
                INK,
            )
            counter(
                "Strat",
                p.strategy,
                lambda d, p=p: (setattr(p, "strategy", max(0, p.strategy + d)), touch()),
                INK,
            )
...
def add_player_dialog() -> None:
    with ui.dialog() as dialog, ui.card().style(f"background:{PANEL}"):
        ...
        faction = ui.select(list(FACTIONS), value=FACTIONS[0], with_input=True).props(
            "dense outlined dark"
        ).classes("w-64")
    ...
    def board() -> None:
        with ui.column().classes("w-full max-w-[1400px] mx-auto gap-4 p-5"):
            ...
            with ui.row().classes("gap-2"):
                ...
                ui.button("New round", on_click=lambda: (game.new_round(), touch())).props(
                    "flat dense no-caps"
                ).classes("imperial text-[10px]").style(f"color:{DIM}")

            if game.log:
                with ui.expansion("Session log").classes("w-full text-xs").style(f"color:{DIM}"):
                    for line in game.log[:25]:
                        ui.label(line).classes("num text-[11px]").style(f"color:{DIM}")
...
if __name__ in {"__main__", "__mp_main__"}:
    ui.run(title="Twilight Imperium", dark=True, favicon="🌌", port=8080)
